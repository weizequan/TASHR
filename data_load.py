...
class Dataset(object):
    def __init__(self, config):
        ...
        # print(self.train_concat_list)
        # self.val_list = self.load_flist(config.VAL_FLIST)
        # self.val_mask_list = self.load_flist(config.VAL_MASK_FLIST)
        # self.val_gt_list = self.load_flist(config.VAL_GT_FLIAT)
    ...
    def data_batch(self):
        ...
        def image_fn(img_path):
            ...
            img = tf.image.resize_images(x_decode, [256, 768])
            # img = tf.cast(x_decode, tf.float32)
            return img

        train_concat = train_concat.map(image_fn, num_parallel_calls=self.batch_size)
        train_dataset = tf.data.Dataset.zip((train_concat))
        train_dataset = train_dataset.apply(tf.data.experimental.shuffle_and_repeat(1000, 10 * self.epoch)).batch(
        ...
        val_dataset = val_dataset.apply(tf.data.experimental.shuffle_and_repeat(1000, 10 * self.epoch)).batch(
            self.val_batch_size, drop_remainder=True)
